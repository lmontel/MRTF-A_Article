...
#        Button(self,text="Renommer les fichiers",command=self.rename).grid(row=2,column=1)
    ...
    def new_exp(self):
        ...
        n=len(path.split("//"))
        self.ExpFrame.exp.date.set(path.split("//")[-4])# La date de l'expérience peut être récupérée à partir d'un nom de dossier dans le chemin
...
class Exp_Frame(Toplevel):
    ...
    def save_in_DB(self):
        ...
        import sqlite3
        DB="//Volumes//DDLorraine//These//DocumentsLorraine//Etirement - Copie - Kaori.sq3"
        ...
        for zonekey in self.exp.zones_sorted:
            ...
            for cellframe in zoneframe.cells:
                ...
                cell=cellframe.cell
                AMC=cell.MCherry.get()
                Cell_loc=cellframe.titre
                tu=(AMC,Cell_loc)+Zone_ID
                n=1
                col="(Actine_MCherry,Cell_loc,Zone_ID"
                val="(?,?,?"
                selection="SELECT Cell_ID FROM Cellules WHERE Zone_ID="+str(Zone_ID[0])+ " AND Actine_MCherry="+str(int(AMC))+ " AND Cell_loc='"+Cell_loc+"'"
...
mainloop()
#//Volumes//DDLorraine//These//DocumentsLorraine//Etirement//2013-09-11//lamelle1//apres//
